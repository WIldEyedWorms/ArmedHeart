...
Shader "Shader Forge/Examples/Vertex Animation" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float node_133 = pow((abs((frac((o.uv0+node_9702.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Vectors:
            ...
            float4 node_9702 = _Time + _TimeEditor;
            float node_133 = pow((abs((frac((i.uv0+node_9702.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
// Final Color:
    ...
    Pass {
        ...
        VertexOutput vert (VertexInput v) {
            ...
            o.tangentDir = normalize( mul( _Object2World, float4( v.tangent.xyz, 0.0 ) ).xyz );
            o.bitangentDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
            float4 node_9804 = _Time + _TimeEditor;
            float node_133 = pow((abs((frac((o.uv0+node_9804.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            v.vertex.xyz += (node_133*_BulgeScale*v.normal);
            o.posWorld = mul(_Object2World, v.vertex);
            float3 lightColor = _LightColor0.rgb;
            o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
...
/// Vectors:
            ...
            float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
            float4 node_9804 = _Time + _TimeEditor;
            float node_133 = pow((abs((frac((i.uv0+node_9804.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
            float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
// Final Color:
    ...
    Pass {
        ...
        VertexOutput vert (VertexInput v) {
            ...
            o.uv2 = v.texcoord2;
            o.normalDir = UnityObjectToWorldNormal(v.normal);
            float4 node_4459 = _Time + _TimeEditor;
            float node_133 = pow((abs((frac((o.uv0+node_4459.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            v.vertex.xyz += (node_133*_BulgeScale*v.normal);
            o.posWorld = mul(_Object2World, v.vertex);
            o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
...
/// Vectors:
    ...
    Pass {
        ...
        VertexOutput vert (VertexInput v) {
            ...
            float node_133 = pow((abs((frac((o.uv0+node_6302.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            v.vertex.xyz += (node_133*_BulgeScale*v.normal);
            o.posWorld = mul(_Object2World, v.vertex);
...
/// Vectors:
            ...
            float4 node_6302 = _Time + _TimeEditor;
            float node_133 = pow((abs((frac((i.uv0+node_6302.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
